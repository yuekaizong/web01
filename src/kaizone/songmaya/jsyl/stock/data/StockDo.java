...
public class StockDo extends JSONResponse {
	...
	public static class Info {
		...
		public static Info[] parse(JSONArray jsonArray) {
			if (jsonArray == null)
				return null;
			...
			try {
				for (int i = 0; i < len; i++) {
					list[i] = StockDo.Info.parse(jsonArray.getJSONObject(i));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static StockDo createStockDo(String symbol, int unit, float open,
			float lastTrade, float prevClose, String volume, String turnover,
		...
		obj.amplitude = Utils.floatTo((obj.zuiGao - obj.zuiDi) / obj.prevClose,
				4);
		sdo.info = obj;
		sdo.success = true;
		sdo.message = "请求成功";
